...
export const configApi = {
  ...
  async getConfig(): Promise<Config> {
    return managementApi.get("/config");
  },

  // Update config
  async updateConfig(config: Config): Promise<Config> {
    return managementApi.put("/config", config);
  },
  ...
  async exportConfig(
    ...
    return managementApi.post("/config/export", {
      config,
  ...
  async importConfig(
    ...
    return managementApi.post("/config/import", {
  ...
  async importBundle(
    ...
    return managementApi.post("/config/import-bundle", {
      current_config: currentConfig,
      content,
      merge,
    });
  ...
  async getConfigPaths(): Promise<ConfigPathInfo> {
    return managementApi.get("/config/paths");
  },

  // Save config to file
  async saveConfig(config: Config): Promise<void> {
    return managementApi.post("/config/save", config);
  },

  // Reload config from file
  async reloadConfig(): Promise<Config> {
    return managementApi.post("/config/reload");
  },
};
